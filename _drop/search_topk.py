...
def search(args):
    ...
    for n in range(start, args.n_generation):
        ...
        sorted_by_fitness = sorted(archive, key=lambda x: np.mean(x["acc_list"]), reverse=False)
        clean_sorted_by_fitness = [{key: value for key, value in agent.items() if key in clean_archive_keys} for agent in sorted_by_fitness]
        system_prompt, prompt = get_prompt(clean_sorted_by_fitness[-7:]) # keep top-k
        ### END - ADDED ###
        msg_list = [
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default='results/drop/topk/')
