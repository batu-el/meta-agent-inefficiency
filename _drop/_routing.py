...
# - removed evaluation from here
# - modified the AgentSystem class to use the CostTracker class
# - modified the evaluate forward function to use the CostTracker class
...
def evaluate_forward_fn(forward_str, test_example):
    ...
    # assert SEARCHING_MODE == False, "SEARCHING_MODE must be False during evaluation"
    ...
    # print(test_examples)
    questions = [example['inputs'] for example in [test_example]]
    answers = [example['targets'] for example in [test_example]]

    assert len(questions) == 1, "questions must be a list of length 1"
...
def routing(args):
    ...
    train_questions = [example['inputs'] for example in train_examples]
    ...
    test_questions = [example['inputs'] for example in test_examples]
    ...
    # print(softmax_probabilities.shape, designed_agent_accuracies.T.shape)
    designed_agent_scores = softmax_probabilities @ designed_agent_accuracies.T
    # print(designed_agent_scores.shape)
    selected_agents = np.argmax(designed_agent_scores, axis=1)
    ...
    sol["designed_agent_scores"] = designed_agent_scores.tolist()
    # save results
    routing_file_path = os.path.join(args.save_dir, f"{args.expr_name}_run_archive.json") .strip(".json") + "_routing_evaluate.json" 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default='results/drop/cumulative/')
    ...
    ### batu: adding routing
    SEARCHING_MODE = False
    for save_dir in ["results/drop/cumulative/", "results/drop/parallel/", "results/drop/topk/"]:
